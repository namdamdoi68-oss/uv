...
/// Merge persisted tool options with the current filesystem configuration.
///
/// Tool receipts contain the effective options from installation, including filesystem proxy
/// declarations. Preserve those higher-precedence routes while retaining genuinely new routes
/// from the current filesystem configuration. Filesystem declarations are validated independently
/// so invalid or duplicate project, user, and system entries remain errors.
